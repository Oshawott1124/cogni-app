...
const electronAPI = {
  ...
  toggleOverlayMode: async () => {
    console.log('toggleOverlayMode called from preload')
    try {
      const result = await ipcRenderer.invoke('toggle-overlay-mode')
      return result
    } catch (error) {
  ...
  setHasContent: (hasContent: boolean) => {
    return ipcRenderer.invoke('set-has-content', hasContent)
  },

  // New: Dynamic mouse ignore control
  setMouseIgnore: (ignore: boolean) => {
    return ipcRenderer.invoke('set-mouse-ignore', ignore)
  },
  
  // New: Listen for overlay mode changes (legacy - now edit mode)
  onOverlayModeChanged: (callback: (isOverlayMode: boolean) => void) => {
    const subscription = (_, isOverlayMode: boolean) => callback(isOverlayMode)
    ipcRenderer.on('overlay-mode-changed', subscription)
    return () => ipcRenderer.removeListener('overlay-mode-changed', subscription)
  },

  // New: Listen for edit mode changes
  onEditModeChanged: (callback: (isEditMode: boolean) => void) => {
    const subscription = (_, isEditMode: boolean) => callback(isEditMode)
    ipcRenderer.on('edit-mode-changed', subscription)
    return () => ipcRenderer.removeListener('edit-mode-changed', subscription)
  },
